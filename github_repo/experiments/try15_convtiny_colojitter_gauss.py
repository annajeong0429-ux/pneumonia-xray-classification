# ══════════════════════════════════════════════════════════════════
# 폐렴 X-ray 분류 | ConvNeXt-Tiny | Kaggle 버전
# Try 18 기준 (최고점 0.9599)
# + TTA 10회 + ColorJitter 강화 + GaussianBlur 추가
...
print("=" * 60)
print("  ConvNeXt-Tiny | Try18 + TTA10 + ColorJitter↑ + GaussBlur")
print("=" * 60)
print(f"  Device   : {DEVICE}")
print(f"  Epochs   : {EPOCHS}  |  Patience : {PATIENCE}")
print(f"  LR       : {LR}  |  TTA : {N_TTA}회")
print(f"  변경사항 : TTA 5→10 | ColorJitter 강화 | GaussianBlur 추가")
...
# ── 6. Transform 정의 ─────────────────────────────────────────────
# X-ray 특성상 밝기/대비 차이가 크므로 ColorJitter 강화
# GaussianBlur로 해상도 차이 시뮬레이션
train_tf = transforms.Compose([
    transforms.Resize((IMG_SIZE, IMG_SIZE)),
    transforms.RandomHorizontalFlip(),
    transforms.RandomRotation(10),
    # ★ ColorJitter 강화: 0.1 → 0.2 (X-ray 밝기 편차 대응)
    transforms.ColorJitter(brightness=0.2, contrast=0.2,
                           saturation=0.1, hue=0.05),
    transforms.RandomAffine(degrees=0, translate=(0.05, 0.05)),
    # ★ GaussianBlur 추가: X-ray 해상도 차이 시뮬레이션
    # kernel_size=3, sigma=(0.1, 1.0) — 약하게 적용
    transforms.RandomApply(
        [transforms.GaussianBlur(kernel_size=3, sigma=(0.1, 1.0))],
        p=0.3  # 30% 확률로만 적용 (과도한 블러 방지)
    ),
    transforms.ToTensor(),
...
# TTA: ColorJitter 추가로 다양한 밝기에서 추론
tta_tf = transforms.Compose([
    transforms.Resize((IMG_SIZE, IMG_SIZE)),
    transforms.RandomHorizontalFlip(),
    transforms.RandomRotation(5),
    # ★ TTA에도 약한 ColorJitter 추가 (밝기 다양성)
...
print("\n  Transform 설정:")
print("  train_tf : HFlip + Rotate(10) + ColorJitter(0.2↑) + RandomAffine")
print("             + GaussianBlur(p=0.3) ← ★신규")
print("  val_tf   : Resize + Normalize (변경 없음)")
print("  tta_tf   : HFlip + Rotate(5) + ColorJitter(0.1) ← ★신규")
...
for fold, (tr_idx, val_idx) in enumerate(
    ...
    torch.save(best_state,
               SAVE_DIR / f"convtiny_tta10_gauss_fold{fold}.pth")
    print(f"  모델 저장 완료 → convtiny_tta10_gauss_fold{fold}.pth")

# ── 12. 최종 결과 요약 ────────────────────────────────────────────
print(f"\n{'='*60}")
print(f"  [Kaggle] ConvNeXt-Tiny | Try18 + TTA10 + ColorJitter↑ + GaussBlur")
...
out_name = "dodo_project-convtiny-tta10-jitter-gauss.csv"
