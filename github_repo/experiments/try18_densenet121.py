# ══════════════════════════════════════════════════════════════════
# 폐렴 X-ray 분류 | DenseNet121 | Colab 버전
# CheXNet 논문 근거: DenseNet121로 폐렴 X-ray 방사선과 의사 수준 달성
...
print("=" * 60)
print("  DenseNet121 | K-Fold 5 | TTA 5 | Colab")
print("=" * 60)
print(f"  Device   : {DEVICE}")
print(f"  IMG_SIZE : {IMG_SIZE}  |  BATCH : {BATCH_SIZE}")
print(f"  Epochs   : {EPOCHS}  |  Patience : {PATIENCE}")
print(f"  LR       : {LR}  |  TTA : {N_TTA}회")
print(f"  파라미터 : 8M  |  예상 시간 : 35~45분")
print(f"  근거     : CheXNet - DenseNet121 폐렴 X-ray 최강")
...
def build_model():
    """
    DenseNet121
    - IMAGENET1K_V1 사전학습 가중치
    - classifier: Linear(1024 → 2)
    - Dense Connection: 모든 레이어가 이전 레이어와 연결
      → 폐 윤곽, 음영 등 저수준 특징이 끝까지 유지됨
    """
...
print(f"  ✓ DenseNet121 파라미터 수: {total_params:.1f}M")
del _m
...
for fold, (tr_idx, val_idx) in enumerate(
    ...
    tr_loader = DataLoader(
        ...
        num_workers=2, pin_memory=True   # Colab: num_workers=2
    )
    ...
    torch.save(best_state,
               SAVE_DIR / f"densenet121_fold{fold}.pth")
    print(f"  모델 저장 완료 → densenet121_fold{fold}.pth")
...
# ── 12. 최종 결과 요약 ────────────────────────────────────────────
print(f"\n{'='*60}")
print(f"  DenseNet121 | 오버샘플링 + K-Fold {N_FOLDS} + TTA {N_TTA}")
...
out_name = "dodo_project-densenet121-kfold-tta.csv"
...
# 앙상블용 확률값 저장
np.save(SAVE_DIR / "densenet121_test_probs.npy",
        test_probs)
print(f"\n  확률값 저장 완료 → densenet121_test_probs.npy (앙상블용)")
